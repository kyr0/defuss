...
export function LoginScreen() {
	...
	const { isValid, getMessages } = transval(
		formRule(form.email).isString().isEmail(),
	...
	const handleSignIn = async () => {
		...
		if (!(await isValid(formValues))) {
			...
			toast({ category: "error", title: t("login.validation_failed"), description: t("login.please_fix_errors") });
			return;
		}
		...
		try {
			...
			if (result.success && result.token && result.user) {
				...
				$(successContainerRef)
					.update(
						<Alert>
							<AlertTitle>{t("login.welcome_back")}</AlertTitle>
							<AlertDescription>{t("login.redirecting")}</AlertDescription>
						</Alert>,
					)
					.removeClass("hidden");
				...
				$(errorContainerRef)
					.update(
						<Alert variant="destructive">
							...
							<AlertDescription>{result.error || t("login.invalid_credentials")}</AlertDescription>
						</Alert>,
					)
					.removeClass("hidden");
				toast({ category: "error", title: t("login.login_failed"), description: result.error || t("login.check_credentials") });
			}
		} catch (error) {
			console.error("Login error:", error);
			toast({ category: "error", title: t("login.unexpected_error"), description: t("login.try_again") });
		}
	};
			...
			<div className="flex flex-col gap-6 mx-5">
				...
				>
					...
					<div className="grid gap-2">
						...
						<Input ref={refs.password.input} id="password" name="password" type="password" placeholder="--------" />
				...
				<div className="relative">
					...
					<div className="relative flex justify-center text-xs uppercase">
						<span className="bg-background px-2 text-muted-foreground">{t("login.demo_credentials")}</span>
					</div>
				</div>
				...
				<div className="rounded-lg border bg-muted/50 p-4 text-sm">
					<div class="grid grid-cols-[auto_1fr_auto] items-center gap-x-3 gap-y-2">
						...
						<CopyValueButton label={t("login.email_label")} value="admin@example.com" />
						...
						<CopyValueButton label={t("login.password_label")} value="Admin123$" />
					</div>
				</div>
			</div>
		</Card>
