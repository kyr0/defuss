...
export class MidiOutAdapter {
  ...
  private gridMs = 0; // duration of selected minimum note in ms
  private winCommitAtMs: number | null = null;
  private gridOriginMs: number | null = null; // anchor for grid boundaries
  private lastCommittedBoundaryIdx: number | null = null; // prevent duplicate ONs per boundary
  private winCommitIdx: number | null = null;
  ...
  start(): void {
    ...
    this.pendingOn = null;
    this.winCommitAtMs = null;
    this.gridOriginMs = null;
    this.lastCommittedBoundaryIdx = null;
    this.winCommitIdx = null;
  ...
  setGridAligned(on: boolean) {
    this.gridAligned = !!on;
    this.winCommitAtMs = null;
  }
  setGridMs(ms: number) {
    this.gridMs = Math.max(0, Math.round(ms));
    this.winCommitAtMs = null;
  }
  ...
  private processEvent(ev: NoteEvent) {
    // Close/commit window either at grid boundary (if enabled) or by window length
    if (this.winActive) {
      if (this.gridActive()) {
        if (this.winCommitAtMs == null) {
          this.winCommitAtMs = this.nextGridBoundary(this.winStartMs);
        }
        if (ev.tMs >= this.winCommitAtMs) {
          this.commitWindow(this.winCommitAtMs);
        }
      } else if (ev.tMs - this.winStartMs >= (this.cfg.windowMs ?? 220)) {
    ...
    switch (ev.event) {
      case "ON": {
        ...
        if (this.metroEnabled && !this.metroRunning) {
          // Anchor grid to the transient that starts the metronome
          this.gridOriginMs = ev.tMs;
          this.lastCommittedBoundaryIdx = null;
          this.winCommitIdx = null;
          this.startMetronome();
        }
        if (!this.winActive) {
          ...
          if (this.gridActive()) {
            // Grid-aligned: schedule commit at next grid boundary; don't emit provisional
            this.winCommitAtMs = this.nextGridBoundary(ev.tMs);
            this.winCommitIdx = this.boundaryIndexFor(this.winCommitAtMs)!;
            this.winProvisionalBase = id;
            this.winProvisionalSent = false;
          } else if (this.cfg.smoothingMode === "provisional") {
      ...
      case "OFF": {
        // OFF: if grid-aligned and before commit time, drop window without emitting
        if (this.winActive && this.gridActive() && this.winCommitAtMs != null) {
          if (ev.tMs < this.winCommitAtMs) {
            this.resetWindow();
          } else {
            this.commitWindow(this.winCommitAtMs);
  ...
  private commitWindow(nowMs: number) {
    ...
    // If grid-aligned, ensure we commit at/after boundary and only once per boundary
    let commitAt = nowMs;
    if (this.gridActive()) {
      const at = this.winCommitAtMs ?? this.nextGridBoundary(this.winStartMs);
      if (nowMs < at) {
        // too early; skip emission
        this.resetWindow();
        return;
      }
      // Prevent multiple ONs within the same grid boundary using integer boundary index
      const idx = this.winCommitIdx ?? this.boundaryIndexFor(at)!;
      if (
        this.lastCommittedBoundaryIdx != null &&
        idx === this.lastCommittedBoundaryIdx
      ) {
        this.resetWindow();
        return;
      }
      this.winCommitIdx = idx;
      commitAt = at;
    }
    const decided = this.decideIdentity(ids, this.cfg.decisionRule);
    const useDelayed =
      this.gridActive() || this.cfg.smoothingMode === "delayed";
    if (!useDelayed) {
      ...
      if (!this.winProvisionalSent) {
        ...
        const enoughTime = commitAt - this.lastOnAtMs >= this.minArticulationMs;
        if (decided !== this.winProvisionalBase && enoughGap && enoughTime) {
          this.emitOff(commitAt);
          this.emitOn(
            {
              tMs: commitAt,
              pitchHz: this.midiToFreq(decided),
    ...
    if (this.gridActive() && this.winCommitIdx != null) {
      this.lastCommittedBoundaryIdx = this.winCommitIdx;
    }
    this.resetWindow();
    this.winCommitAtMs = null;
    this.winCommitIdx = null;
  ...
  private nextGridBoundary(tMs: number): number {
    const g = this.gridMs | 0;
    if (g <= 0) return tMs;
    const origin = this.gridOriginMs;
    if (origin == null) return tMs; // grid inactive until anchored
    const rel = tMs - origin;
    if (rel <= 0) return origin; // first boundary is the origin itself
    return origin + Math.ceil(rel / g) * g;
  }

  private boundaryIndexFor(tMs: number): number | null {
    if (!this.gridActive()) return null;
    const g = this.gridMs | 0;
    const origin = this.gridOriginMs!;
    const rel = tMs - origin;
    if (rel <= 0) return 0;
    return Math.ceil(rel / g);
  }

  private gridActive(): boolean {
    return this.gridAligned && this.gridMs > 0 && this.gridOriginMs != null;
  }
