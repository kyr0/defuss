...
export function DashboardScreen() {
  ...
  const gridRef = createRef<null, HTMLInputElement>();
  ...
  const setGridAligned = (on: boolean) => {
    if (gridRef.current) gridRef.current.checked = on;
    midi.setGridAligned(on);
  };
  ...
  const applyTimingDerived = () => {
    ...
    // Grid duration equals selected minimum note duration
    midi.setGridMs(Math.round(dur));
  };
  ...
  const onGridToggle = () => {
    const on = !!gridRef.current!.checked;
    setGridAligned(on);
  };
    ...
    <div class="w-full max-w-xl space-y-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="flex gap-x-3 ml-4 items-center justify-center">
          ...
          />
          <label class="uk-form-label mt-1" for="gridAligned">
            Grid aligned
          </label>
        </div>
      </div>
