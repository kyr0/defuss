  ...
  return (
    <>
      {notebookFiles.map(({ notebook }) => (
        <li><a key={notebook.id} class="link notebook-link" href="#" onClick={(e) => onNotebookLinkClick(e, notebook)}>{notebook.title}</a></li>
      ))}
    </>
  )
}
...
export function Notebooks({ tocRef, onNotebookLinkClick }: NotebookListProps) {
  ...
  const InnerList = () => (<Async fallback={<div>Loading...</div>}>
    ...
    <>
      <header class="p-xs vbox items-center justify-between">
        ...
        <div class="logo vbox items-center gap-xs" style={{ width: 'auto' }}>
          <img src="/defuss_mascott.webp" alt="defuss Mascott" style="height: auto; width: 40px" />
          <h1 class="h4">defuss <span class="h5 dim">notebooks</span></h1>
        ...
        <div class="vbox gap-2xxs">
          <button type="button" class="btn rounded" onClick={toggleReadOnlyAndEditMode}>
            📝
          </button>
          <button type="button" class="btn rounded hidden" ref={saveBtnRef} onClick={switchSourceAndWysiwyg}>
            💾
          </button>
        </div>
      ...
      <main class="vbox fit">
        ...
        <section class="fit relative">
          <div class="rounded p-xs sheet h-full px-2xl mx-xs overflow-auto pb-xxl pt-xl" ref={sheetRef}>
            <h5>How to use this?</h5>
            <p>
              Open a notebook and play with the code.
            </p>
          </div>
        </section>
