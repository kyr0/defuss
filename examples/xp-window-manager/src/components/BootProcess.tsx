...
import { internetExplorerAppBundle, notepadAppBundle } from "../apps/index.js";
...
export function BootProcess({ bootDelayMs = 0 }: BootProcessProps) {
  ...
  const onLogon = async () => {
    ...
    if (!desktopShell.findApp(notepadAppBundle.executable)) {
      await desktopShell.registerAppBundle(notepadAppBundle);
    }

    if (!desktopShell.findApp(internetExplorerAppBundle.executable)) {
      await desktopShell.registerAppBundle(internetExplorerAppBundle);
    }
