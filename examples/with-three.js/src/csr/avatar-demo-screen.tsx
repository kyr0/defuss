			...
			<div class="meter-footer muted-note">
				...
				<div>
					{meterState.listening
						...
						: "Start Self-Test to stream the mic into the selected backend."}
				</div>
			</div>
		</div>
	);
...
async function startSelfTest() {
	...
	logLine(`Self-Test started with ${backend.label} (mic ${nativeRate} Hz -> ${TARGET_SAMPLE_RATE / 1000} kHz, hop ${hopSize}).`);
	setStatus("Self-Test", `Speak into the microphone - ${backend.label} drives the mouth and the live voice/noise meters.`);
}
...
async function handleBackendChange() {
	...
	if (mic.active) {
		await stopSelfTest();
		setStatus("Backend changed", `Stopped the live stream. Restart Self-Test to use ${backend.label}.`, "warning");
	}
				...
				<CardHeader>
					<div class="flex items-center justify-between gap-3">
						<div>
							...
							<CardDescription>
								three.js + defuss-vad. <strong>Offline:</strong> analyse a WAV then play with backend-guided lip-sync. <strong>Self-Test:</strong> stream the mic through TenVAD, FireRed, or Silero and watch live voice vs. noise meters.
							</CardDescription>
						</div>
					</div>
				</CardHeader>
