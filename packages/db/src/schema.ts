...
export function matchesSelector<T extends DefussRecord>(
  ...
  for (const [field, expectedValue] of Object.entries(selector)) {
    ...
    const currentValue = field === "id" ? record.id : getValueAtPath(record, field);
...
function deriveIndexValue<T extends DefussRecord>(
  ...
  const rawValue =
    typeof source === "function" ? source(value) : (getValueAtPath(value, source) as RecordValue | undefined);
