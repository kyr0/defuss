...
describe("Memory Allocation Comparison", () => {
  beforeAll(async () => {
    ...
    console.log(
      `🧵 WASM initialized with ${navigator.hardwareConcurrency} threads`,
    );
  });
  it("should demonstrate fast memory allocation benefits", async () => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      if (i % 2 === 0) {
        const result = vector_dot_product(
          smallData.vectorsA,
          smallData.vectorsB,
        );
        results.push(result);
    ...
    console.log(
      `⏱️  Completed ${iterations} iterations in ${totalTime.toFixed(2)}ms`,
    );
    ...
    console.log(
      `✅ Pool hits: ${stats.bufferPool.hits}, ❌ Pool misses: ${stats.bufferPool.misses}`,
    );

    // We should see either stack usage OR pool usage (or both)
    const hasStackUsage = stats.stackUsage > 0;
    const hasPoolActivity =
      stats.bufferPool.hits > 0 || stats.bufferPool.misses > 0;
    ...
    if (stats.bufferPool.misses > 5) {
      ...
      console.log(
        "🔍 Pool was used but hit rate may be low due to test pattern",
      );
    }
  });
  ...
  it("should compare memory allocation strategies", async () => {
    ...
    console.log(
      `   - Buffer pool utilization: ${finalStats.bufferPool.hitRate}%`,
    );
    console.log(
      `   - Stack usage: ${finalStats.stackUsage}/${finalStats.stackSize} bytes`,
    );
    console.log(
      `   - Pool hits: ${finalStats.bufferPool.hits}, misses: ${finalStats.bufferPool.misses}`,
    );
    ...
    const hasMemoryOptimizations =
      finalStats.stackUsage > 0 || finalStats.bufferPool.misses > 0;
    expect(hasMemoryOptimizations).toBe(true);
  });

  it("should perform 100k 1024D vector multiplications efficiently", async () => {
    console.log(
      "🚀 Testing 100k vector multiplications with 1024 dimensions...",
    );
    ...
    console.log(
      `✅ Completed ${totalOperations.toLocaleString()} vector multiplications`,
    );
    console.log(`⏱️  Total time: ${totalTime.toFixed(2)}ms`);
    console.log(
      `⚡ Operations per second: ${(totalOperations / (totalTime / 1000)).toLocaleString()}`,
    );
    console.log(
      `🎯 Average time per operation: ${((totalTime / totalOperations) * 1000).toFixed(3)}μs`,
    );
  ...
  it("should compare JS vs WASM single vs WASM parallel for 100k operations", async () => {
    console.log(
      "🔥 Comparing JS vs WASM performance for 100k 1024D vector multiplications...",
    );
    ...
    console.log(
      `📝 Test setup: ${vectorCount} vectors × ${batches} batches = ${totalOperations.toLocaleString()} operations`,
    );
    ...
    console.log(
      `✅ JS completed: ${jsTime.toFixed(2)}ms (${jsOpsPerSec.toLocaleString()} ops/sec)`,
    );
    ...
    console.log(
      `✅ WASM Adaptive completed: ${wasmAdaptiveTime.toFixed(2)}ms (${wasmAdaptiveOpsPerSec.toLocaleString()} ops/sec)`,
    );
    ...
    console.log(
      `✅ WASM Single completed: ${wasmSingleTime.toFixed(2)}ms (${wasmSingleOpsPerSec.toLocaleString()} ops/sec)`,
    );
    ...
    console.log(
      `✅ WASM Parallel completed: ${wasmParallelTime.toFixed(2)}ms (${wasmParallelOpsPerSec.toLocaleString()} ops/sec)`,
    );
    ...
    console.log(
      "│ Implementation      │ Time (ms)    │ Ops/sec         │ Speedup      │",
    );
    console.log(
      "├─────────────────────┼──────────────┼─────────────────┼──────────────┤",
    );
    ...
    const times = [
      ...
      {
        name: "WASM Adaptive",
        time: wasmAdaptiveTime,
        ops: wasmAdaptiveOpsPerSec,
      },
      { name: "WASM Single", time: wasmSingleTime, ops: wasmSingleOpsPerSec },
      {
        name: "WASM Parallel",
        time: wasmParallelTime,
        ops: wasmParallelOpsPerSec,
      },
    ];
    ...
    times.forEach(({ name, time, ops }) => {
      ...
      console.log(
        `│ ${name.padEnd(19)} │ ${time.toFixed(2).padStart(10)} │ ${Math.round(ops).toLocaleString().padStart(13)} │ ${speedup.toFixed(2).padStart(10)}x │`,
      );
    });
    ...
    // Find the winner
    const winner = times.reduce((prev, current) =>
    ...
    console.log(
      `📈 Ratio vs 35ms target: ${vsTarget.toFixed(2)}x ${vsTarget < 1 ? "(FASTER!)" : "(slower)"}`,
    );
  });
});
