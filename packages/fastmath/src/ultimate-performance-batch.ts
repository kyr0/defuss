...
export class UltimatePerformanceBatch {
  ...
  generateTestDataCacheOptimized(
    ...
    for (let chunkStart = 0; chunkStart < totalSize; chunkStart += cacheLineSize) {
      ...
      for (; i < chunkEnd - 15; i += 16) {
        vectorsA[i] = seededRandom();     vectorsB[i] = seededRandom();
        vectorsA[i + 1] = seededRandom(); vectorsB[i + 1] = seededRandom();
        vectorsA[i + 2] = seededRandom(); vectorsB[i + 2] = seededRandom();
        vectorsA[i + 3] = seededRandom(); vectorsB[i + 3] = seededRandom();
        vectorsA[i + 4] = seededRandom(); vectorsB[i + 4] = seededRandom();
        vectorsA[i + 5] = seededRandom(); vectorsB[i + 5] = seededRandom();
        vectorsA[i + 6] = seededRandom(); vectorsB[i + 6] = seededRandom();
        vectorsA[i + 7] = seededRandom(); vectorsB[i + 7] = seededRandom();
        vectorsA[i + 8] = seededRandom(); vectorsB[i + 8] = seededRandom();
        vectorsA[i + 9] = seededRandom(); vectorsB[i + 9] = seededRandom();
        vectorsA[i + 10] = seededRandom(); vectorsB[i + 10] = seededRandom();
        vectorsA[i + 11] = seededRandom(); vectorsB[i + 11] = seededRandom();
        vectorsA[i + 12] = seededRandom(); vectorsB[i + 12] = seededRandom();
        vectorsA[i + 13] = seededRandom(); vectorsB[i + 13] = seededRandom();
        vectorsA[i + 14] = seededRandom(); vectorsB[i + 14] = seededRandom();
        vectorsA[i + 15] = seededRandom(); vectorsB[i + 15] = seededRandom();
      }
  ...
  async ultimatePerformanceTest(
    ...
    const { vectorsA, vectorsB } = this.generateTestDataCacheOptimized(numVectors, vectorLength, seed);
    ...
    if (numVectors <= 50000) {
      ...
      results = this.batchDotProductPureJS(vectorsA, vectorsB, vectorLength, numVectors);
      method = "Pure JS SIMD-style";
    } else {
      // For larger datasets, use chunked processing
      const wasmResult = await this.batchDotProductWasmFixed(vectorsA, vectorsB, vectorLength, numVectors);
...
class MemoryPool {
  ...
  generateAndProcessInPlace(actualVectors: number, seed = 42): Float32Array {
    if (actualVectors > this.maxVectors) {
      throw new Error(`Cannot process ${actualVectors} vectors, max is ${this.maxVectors}`);
    }
    ...
    for (let vecIdx = 0; vecIdx < actualVectors; vecIdx++) {
      ...
      for (; j < this.vectorLength - 15; j += 16) {
        ...
        sum += this.vectorsA[baseIdx + j + 10] * this.vectorsB[baseIdx + j + 10];
        sum += this.vectorsA[baseIdx + j + 11] * this.vectorsB[baseIdx + j + 11];
        sum += this.vectorsA[baseIdx + j + 12] * this.vectorsB[baseIdx + j + 12];
        sum += this.vectorsA[baseIdx + j + 13] * this.vectorsB[baseIdx + j + 13];
        sum += this.vectorsA[baseIdx + j + 14] * this.vectorsB[baseIdx + j + 14];
        sum += this.vectorsA[baseIdx + j + 15] * this.vectorsB[baseIdx + j + 15];
      }
  ...
  getMemoryUsage(): { totalMB: number; usedMB: number } {
    const totalBytes = (this.vectorsA.byteLength + this.vectorsB.byteLength + this.results.byteLength);
