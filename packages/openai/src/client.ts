...
import { parseZyphraToolCalls } from './zyphra.js';
import type {
	ChatParams,
	ChatResponse,
	ChatStreamChunk,
...
export const createClient = (config: ClientConfig = {}): OpenAIClient => {
	...
	const createChatCompletion = async (
		...
		// Parse zyphra tool calls if present
		for (const choice of response.choices) {
			const msg = choice.message;
			if (msg.content && !msg.tool_calls?.length) {
				const parsed = parseZyphraToolCalls(msg.content);
				if (parsed.length > 0) {
					msg.tool_calls = parsed;
					msg.content = msg.content.replace(
						/<zyphra_tool_call>[\s\S]*?<\/zyphra_tool_call>\n?/g,
						'',
					).trim() || null;
				}
			}
