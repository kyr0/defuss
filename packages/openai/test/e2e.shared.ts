...
import { getEnv, load } from "defuss-env";
import { createClient, type ChatTool } from "../src/index.js";

// Load .env before reading values (safe no-op in browser since defuss-env guards Node-only)
...
function chat(
	...
	return modelClient.createChatCompletion({
		model: MODEL,
...
export function registerTests() {
	...
	describe("chat completions", () => {
		it("returns a response for a simple prompt", async () => {
			...
			console.log("[chat] response:", JSON.stringify(res.choices[0]?.message, null, 2));
			expect(res.choices.length).toBeGreaterThan(0);
			expect(res.choices[0]!.message.content).toBeTruthy();
		});

		it("handles system + user messages", async () => {
			const res = await modelClient.createChatCompletion({
				model: MODEL,
				messages: [
					{
						role: "system",
			...
			console.log("[system+user] response:", JSON.stringify(res.choices[0]?.message, null, 2));
			expect(res.choices[0]!.message.content).toBeTruthy();
		});
		...
		it("generates code", async () => {
			...
			console.log("[code] response:", JSON.stringify(res.choices[0]?.message, null, 2));
		...
		it("writes creative content (haiku)", async () => {
			...
			console.log("[haiku] response:", JSON.stringify(res.choices[0]?.message, null, 2));
			expect(res.choices[0]!.message.content).toBeTruthy();
		});
		...
		it("writes creative content (limerick)", async () => {
			...
			console.log("[limerick] response:", JSON.stringify(res.choices[0]?.message, null, 2));
			expect(res.choices[0]!.message.content).toBeTruthy();
		});
	});
	...
	describe("streaming chat completions", () => {
		it("streams chunks with delta content", async () => {
			...
			console.log("[stream] chunks:", chunks.length, "full:", JSON.stringify(full));
	...
	describe("sampling parameters", () => {
		it("temp=0 (greedy)", async () => {
			...
			console.log("[temp=0] response:", JSON.stringify(res.choices[0]?.message, null, 2));
			expect(res.choices[0]!.message.content).toBeTruthy();
		});
		...
		it("temp=1.5 (high creativity)", async () => {
			...
			console.log("[temp=1.5] response:", JSON.stringify(res.choices[0]?.message, null, 2));
			expect(res.choices[0]!.message.content).toBeTruthy();
		});
		...
		it("top_p=0.1 (narrow nucleus)", async () => {
			...
			console.log("[top_p=0.1] response:", JSON.stringify(res.choices[0]?.message, null, 2));
			expect(res.choices[0]!.message.content).toBeTruthy();
		});
		...
		it("temp=0 + top_p=1 (deterministic)", async () => {
			...
			console.log("[temp=0+top_p=1] response:", JSON.stringify(res.choices[0]?.message, null, 2));
			expect(res.choices[0]!.message.content).toBeTruthy();
		});
		...
		it("high temp + low top_p", async () => {
			...
			console.log("[high temp+low top_p] response:", JSON.stringify(res.choices[0]?.message, null, 2));
			expect(res.choices[0]!.message.content).toBeTruthy();
		});
	});
	...
	describe("tool calling", () => {
		it("calls get_weather tool", async () => {
			...
			console.log("[get_weather] response:", JSON.stringify(msg, null, 2));
			...
			const followUp = await modelClient.createChatCompletion({
				model: MODEL,
				messages: [
					{
						role: "user",
		...
		it("calls calculate tool for math", async () => {
			...
			const msg = res.choices[0]!.message;
			console.log("[calculate] response:", JSON.stringify(msg, null, 2));
			const toolCalls = msg.tool_calls ?? [];
		...
		it("handles multi-tool request", async () => {
			...
			const msg = res.choices[0]!.message;
			console.log("[multi-tool] response:", JSON.stringify(msg, null, 2));
			const toolCalls = msg.tool_calls ?? [];
			expect(toolCalls.length).toBeGreaterThan(0);
