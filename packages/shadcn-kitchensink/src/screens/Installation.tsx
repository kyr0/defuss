  ...
  return (
    <div class="space-y-6">
      ...
      <section class="space-y-3">
        ...
        <div class="rounded-md bg-muted p-4 overflow-x-auto">
          <pre>
            <code class="text-sm">{`// vite.config.ts
...
export default defineConfig({
  plugins: [defuss(), tailwindcss()],
      ...
      <section class="space-y-3">
        ...
        <p class="text-muted-foreground">
          Create <code>src/css/index.css</code> and import Tailwind + Basecoat
          (required).
        </p>
        <div class="rounded-md bg-muted p-4 overflow-x-auto">
          <pre>
...
@import "./custom.css";`}</code>
      ...
      <section class="space-y-3">
        ...
        <div class="rounded-md bg-muted p-4 overflow-x-auto">
          <pre>
            <code class="text-sm">{`// src/main.tsx
import "./css/index.css";`}</code>
          </pre>
        </div>
        <p class="text-muted-foreground">
          Without this import, components render unstyled even if packages are
          installed.
        </p>
      </section>
      ...
      <section class="space-y-3">
        ...
        <div class="rounded-md bg-muted p-4 overflow-x-auto">
          <pre>
            <code class="text-sm">pnpm dev</code>
          </pre>
        </div>
        <p class="text-muted-foreground">
          Open a screen such as <code>/components/button</code> to confirm
          styles are loaded.
        </p>
      </section>

      <div class="rounded-md border border-primary/30 bg-primary/5 p-4 text-sm">
        <strong>Important:</strong> Installing <code>defuss-shadcn</code> alone
        is not enough. You must include the CSS imports shown above.
      </div>
    </div>
  );
};
