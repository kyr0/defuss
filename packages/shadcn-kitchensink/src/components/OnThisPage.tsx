...
const ensureHeadingIds = (content: HTMLElement) => {
  ...
  const usedIds = new Set(
    Array.from(content.querySelectorAll("[id]")).map((element) => element.id),
  );
...
const collectTocItems = (): TocItem[] => {
  ...
  if (filtered.length === 0 && h1?.id) {
    return [
      { id: h1.id, label: (h1.textContent || "Overview").trim() || "Overview" },
    ];
  }
...
export const OnThisPage: FC = () => {
  ...
  return (
    <div
      ref={rootRef}
      class="hidden text-sm xl:block w-full max-w-[200px]"
      onMount={onMount}
      onUnmount={onUnmount}
    >
      <nav
