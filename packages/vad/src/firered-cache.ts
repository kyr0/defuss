...
const FIRERED_CACHE_TABLE_NAME = "firered_assets";

interface FireRedAssetCacheRecord extends DefussRecord {
	cacheKey: string;
	assetUrl: string;
	bytes: ArrayBuffer;
...
const fireredCacheTable = defineTable<FireRedAssetCacheRecord>({
	name: FIRERED_CACHE_TABLE_NAME,
	indexes: [
		{
			name: "cacheKey",
			source: "cacheKey",
...
const providerPromises = new Map<
	string,
	Promise<DefussTable<FireRedAssetCacheRecord, object> | null>
>();
...
async function getBrowserCacheProvider(
	...
	const next = (async (): Promise<DefussTable<FireRedAssetCacheRecord, object> | null> => {
		const { DexieProvider } = await import("defuss-db/client.js");
		const provider = new DexieProvider(databaseName) as any;
		await provider.connect();
		const table = new DefussTable(provider, fireredCacheTable);
		await table.init();
		return table;
	})();
...
export async function readBrowserCachedFireRedAsset(options: {
	databaseName: string;
	cacheKey: string;
...
export async function writeBrowserCachedFireRedAsset(options: {
	databaseName: string;
	cacheKey: string;
	assetUrl: string;
	bytes: Uint8Array;
