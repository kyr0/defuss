	...
	for (let blockSize = 2; blockSize <= size; blockSize <<= 1) {
		...
		for (let blockStart = 0; blockStart < size; blockStart += blockSize) {
			...
			for (let index = blockStart; index < blockStart + halfSize; index++) {
				...
				const currentImag = imag[index]!;

				real[matchIndex] = currentReal - tempReal;
				imag[matchIndex] = currentImag - tempImag;
				real[index] = currentReal + tempReal;
				imag[index] = currentImag + tempImag;
