...
import { getRenderer } from "./isomorph.js";
...
describe("VirtualDOM", () => {
  ...
  it("can render to document.body", () => {
    ...
    expect(
      (renderSync(<div ref={divRef} id="foo" />) as Element).nodeName,
    ).toEqual("DIV");
    expect(divRef.current.nodeName).toEqual("DIV");
    expect(divRef.current.parentElement?.querySelector("#foo")).toEqual(
      divRef.current,
    );
  });
  ...
  it("calls the onMount lifecycle hook when a DOM element has been rendered in to another <div>", () => {
    const someParentDivRef = createRef<HTMLDivElement>();
    const someDivRef = createRef();
...
describe("getRenderer", () => {
  ...
  describe("createElementOrElements", () => {
    it("returns an array of child elements if virtualNode is an array", () => {
      ...
      const result = renderer.createElementOrElements(mockVirtualNode) as Array<
        RenderResult<RenderInput>
      >;
...
describe("readme", () => {
  ...
  it("whole doc", () => {
    ...
    const html: string = renderToString(dom);
    expect(html).toEqual('<html lang="en"><head></head><body></body></html>');
  });
});
...
describe("dangerouslySetInnerHTML", () => {
  ...
  it("renders full document with dangerouslySetInnerHTML in SSR", () => {
    ...
    expect(html).toEqual(
      '<html lang="en"><head></head><body><main><h1>Title</h1><p>Welcome!</p></main></body></html>',
    );
  });
});
