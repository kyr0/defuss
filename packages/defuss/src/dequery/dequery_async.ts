...
// --- Types & Helpers ---
export type aNodeType = Node | Text | Element | Document | DocumentFragment | HTMLElement | SVGElement;
export type aFormFieldValue = string | Date | File | boolean | number;
export interface aFormKeyValues { [keyOrPath: string]: aFormFieldValue; }

const elementGuard = <T = HTMLElement>(el: aNodeType): T => {
  if (el instanceof HTMLElement) return el as T;
  console.warn("Expected HTMLElement, but found:", el?.nodeName ?? el);
  return el as T;
};

export interface aDequeryOptions {
  maxWaitMs?: number;
  timeout?: number;
  delay?: number;
  initialEl?: aNodeType;
...
export const adefaultConfig: aDequeryOptions = {
  maxWaitMs: 1000,
  timeout: 5000,
  delay: 0,
  verbose: false,
...
export type aElementCreationOptions = JSX.HTMLAttributesLowerCase & JSX.HTMLAttributesLowerCase & { html?: string, text?: string };
...
interface DequeryArrayLike {
  [index: number]: aNodeType;
}

class CallChainImpl implements DequeryArrayLike {
  [index: number]: aNodeType;

  options: aDequeryOptions;
  callStack: Array<Call>;
  chainStack: Array<Array<Call>>;
  elements: Array<aNodeType>;
  prevElements: Array<aNodeType>;
  stoppedWithError: boolean;
  lastResult: any;
  isResolved: boolean;

  constructor(options: aDequeryOptions = {}, isResolved = false) {
    ...
    const proxy = new Proxy(this, {
      get(target, prop) {
        if (typeof prop === 'string' && !Number.isNaN(Number(prop))) {
          return target.elements[Number(prop)] as aNodeType;
        }
  ...
  createSubChain(clone = true) {
    if (clone) {
      const newChain = new aDequeryChain({ ...this.options, initialEl: this.getFirstElement() }, true);
      ...
      return new aDequeryChain({ ...this.options, initialEl: this.getFirstElement() }, true);
    }
  }

  // --- Initialization ---
  __init(selectorHtmlRefOrElementRef: string | aNodeType | Ref<aNodeType, any> | RenderInput,
    options: aDequeryOptions | aElementCreationOptions
  ) {
    ...
    if (typeof selectorHtmlRefOrElementRef === 'string') {
      if (selectorHtmlRefOrElementRef.indexOf('<') === 0) {
        ...
        const { text, html, ...attributes } = options as aElementCreationOptions;
      ...
      this.elements = [selectorHtmlRefOrElementRef as aNodeType];
      return this.createSubChain();
      ...
      this.elements = (typeof renderResult !== "undefined" ? Array.isArray(renderResult) ? renderResult : [renderResult] : []) as aNodeType[];
      return this.createSubChain();
    }
  ...
  // --- Traversal ---
  debug(cb: (el: aNodeType | Array<aNodeType>) => void) {
  ...
  ref(ref: Ref<aNodeType>) {
    this.callStack.push(new Call(async function ref(this: CallChainImpl, refObj: Ref<aNodeType>) {
  ...
  all() {
    this.callStack.push(new Call(function all(this: CallChainImpl) {
      return this.elements.map(el => new aDequeryChain({ ...this.options, initialEl: el }, true));
    }));
    return this;
  }
  ...
  replaceWith(content: string | RenderInput | aNodeType | Ref<aNodeType> | typeof aDequeryChain) {
    this.callStack.push(new Call(async function replaceWith(this: CallChainImpl, content: string | RenderInput | aNodeType | Ref<aNodeType> | typeof aDequeryChain) {
      let newElement: aNodeType;
      ...
      if (typeof content === 'string') {
        ...
        newElement = renderIsomorphicSync(content as RenderInput, undefined, globalThis as Globals) as aNodeType;
      } else if (isRef(content)) {
        await waitForRef(content as Ref, this.options.maxWaitMs!);
        newElement = elementGuard(content.current!);
      } else if (content instanceof Node) {
        newElement = content;
      } else if (content instanceof aDequeryChain) {
  ...
  append(content: string | aNodeType | typeof aDequeryChain) {
    this.callStack.push(new Call(function append(this: CallChainImpl, c: string | aNodeType | typeof aDequeryChain) {
      this.elements.forEach(el => {
        if (typeof c === 'string') {
          elementGuard(el).innerHTML += c;
        } else if (c instanceof aDequeryChain) {
  ...
  appendTo(target: aNodeType | Ref<aNodeType> | string | typeof aDequeryChain) {
    this.callStack.push(new Call(async function appendTo(this: CallChainImpl, target: aNodeType | Ref<aNodeType> | string) {
      let targetElement: aNodeType;
      ...
      if (isRef(target)) {
        ...
        targetElement = elementGuard(document.querySelector(target) as aNodeType);
      } else if (target instanceof Node) {
        targetElement = elementGuard(target);
      } else if (target instanceof aDequeryChain) {
  ...
  map(cb: (el: aNodeType, idx: number) => any) {
...
// --- Proxy Wrapper ---
export const aDequeryChain = new Proxy(CallChainImpl, {
...
// --- Entry Points ---
export function adequery(selectorRefOrEl: any, options: aDequeryOptions | aElementCreationOptions = adefaultConfig) {

  let promise = new aDequeryChain({ ...options, initialEl: undefined }).__init(selectorRefOrEl, options);
...
export const a$ = adequery;
export const ad = adequery;

export type aDequery = CallChainImpl;

export function isADequery(obj: unknown): obj is aDequery {
...
async function waitForSelector(
  ...
  return waitFor(() => document.querySelector(sel), timeout, interval);
}
...
async function waitForRef<T>(
  ...
  return waitFor(() => ref.current, timeout, interval);
}
...
function runWithTimeGuard(timeout: number, fn: Function, args: any[], onError: (ms: number, call: Call) => void): Promise<any> {
  return fn(...args);
}
