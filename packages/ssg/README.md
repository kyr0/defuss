...
```bash
npx defuss-ssg -c content -o dist
```
...
const ssg = new SSG({
  ...
  siteDescription: 'A static site generated with defuss-ssg',
...
> `defuss-ssg` is a CLI tool and library for building static websites using modern JavaScript/TypeScript and `defuss`. It reads content files (Markdown, MDX, JSON) from a specified directory, processes them with optional templating (Liquid/Jinja2), and outputs a fully static site ready for deployment.
> 
