...
**Ziel.** Die **Kontinuierliche Kausalitätsdynamik (KKD)** bietet einen prägnanten, leistungsfähigen Formalismus, der Kausalität als kontinuierlichen, zeitlich geordneten Prozess versteht. Sie integriert deterministische, stochastische und zyklische Zusammenhänge in einem einzigen, quellenfreien Digraph-Modell mit klar definierten Axiomen.
...
> „Jede Wirkung hat ihre Ursache, und jede Ursache bringt eine Wirkung hervor.“
...
1. Erweiterte Axiomatik inkl. Stochastizität und Zeitordnung.
2. Graphentheoretische Modellierung mit zeitlicher Entfaltung.
3. Analyse von vier kosmologischen Topologien.
4. Quantitative Metriken: Pfadfrequenzspektren.
5. Python-Simulation zur Untersuchung der Pfaddämpfung.
6. Physikalische Validierung: Relativität, Quantenmechanik, Thermodynamik, Causal Sets.
...
### 4.1 Branching‑DAG (Ewige Inflation)

Quellenfreier, azyklischer Graph mit unendlichen Pfadlängen. Modelliert Multiversum-Entstehung durch permanente Verzweigung.

```mermaid
graph TD
  subgraph Branching_DAG
    y1((y1)) --> x1((x1))
    x1 --> y2((y2))
    x1 --> z1((z1))
  end
...
Einmaliger Ursprung als singuläre Wurzel, monotones Pfadwachstum.
...
KKD stellt einen universellen Rahmen bereit, der Ursache‑Wirkungs‑Netzwerke aller Komplexitätsebenen präzise beschreibt und empirisch verankert.
