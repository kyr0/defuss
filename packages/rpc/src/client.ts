...
function normalizeBinarySource(source: BinarySource): Blob | Uint8Array {
...
function getSourceSize(source: Blob | Uint8Array): number {
...
function sliceBinarySource(
  ...
  return offset > 0 ? source.subarray(offset) : source;
}
...
): Promise<{ body: Blob | Uint8Array; usedCompression: "gzip" | "none" }> {
  if (preference === "none") {
...
export async function* upload<T = unknown>(
  ...
  let requestBody: Blob | Uint8Array | ReadableStream<Uint8Array>;
