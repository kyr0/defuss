 ...
 */
function generateTestData(size: number): Uint8Array<ArrayBuffer> {
...
function createLiteralData(bytes: number[]): Uint8Array<ArrayBuffer> {
	const data = new Uint8Array(new ArrayBuffer(bytes.length));
	data.set(bytes);
	return data;
}

function normalizeBrowserBytes(data: Uint8Array): Uint8Array<ArrayBuffer> {
	const normalized = new Uint8Array(new ArrayBuffer(data.byteLength));
	normalized.set(data);
	return normalized;
}
...
describe("Upload API — browser integration", () => {
	...
	it("should support resumable uploads via HEAD + POST with offset", async () => {
		...
		const firstHalf = normalizeBrowserBytes(data.slice(0, 1024));
