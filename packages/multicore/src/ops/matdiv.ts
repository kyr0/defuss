...
export const matdiv = <T extends NumericArray>(
  ...
  const rows = A.length;
  if (rows !== B.length) {
    throw new RangeError(`matdiv: row count mismatch (${rows} vs ${B.length})`);
  }
  if (rows === 0) return [] as unknown as Matrix<T>;

  const cols = A[0].length;
  const factor = options?.unroll ?? selectUnrollFactor(cols);
  const kernel = getElementWiseKernel("div", factor);
