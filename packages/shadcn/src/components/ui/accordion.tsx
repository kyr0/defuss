...
export const Accordion: FC<AccordionProps> = ({
  ...
  const accordionRef = ref || createRef<HTMLElement>();

  return (
    <section
      ref={accordionRef}
      class={cn("accordion", className)}
      onMount={() => initAccordion(accordionRef.current!, type, collapsible)}
      {...props}
    >
      {children}
    </section>
...
export const AccordionTrigger: FC<AccordionTriggerProps> = ({
  ...
  <summary
    class={cn(
      "list-none [&::-webkit-details-marker]:hidden w-full cursor-pointer px-3 py-3 text-left text-sm font-medium focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] transition-all outline-none rounded-md",
      className,
    )}
    {...props}
  >
...
export const AccordionContent: FC<AccordionContentProps> = ({
  ...
  <section
    class={cn("px-3 pb-3 pt-0 text-sm text-muted-foreground", className)}
    {...props}
  >
    {children}
  </section>
