...
const parsePlacement = (placement?: PopoverPlacement) => {
  if (!placement) return {};
...
const initPopover = (popoverComponent: HTMLDivElement) => {
  const trigger = popoverComponent.querySelector(
    ":scope > button",
  ) as HTMLButtonElement;
  const content = popoverComponent.querySelector(
    ":scope > [data-popover]",
  ) as HTMLElement;
  ...
  const rootId =
    popoverComponent.id || `popover-${Math.random().toString(36).slice(2, 9)}`;
...
export const Popover: FC<PopoverProps> = ({
  ...
  const popoverRef = ref || createRef<HTMLDivElement>();
...
export const PopoverContent: FC<PopoverContentProps> = ({
  ...
  const placementAttrs = parsePlacement(placement);
  return (
    <div
      {...props}
      {...placementAttrs}
