...
const initSidebar = (sidebarComponent: HTMLElement) => {
  ...
  const initialMobileOpen =
    (sidebarComponent as any).dataset.initialMobileOpen === "true";
  const breakpoint =
    parseInt((sidebarComponent as any).dataset.breakpoint, 10) || 768;
...
export const Sidebar: FC<SidebarProps> = ({
  ...
  const sidebarRef = ref || createRef<HTMLElement>();
  ...
  return (
    <aside
      ...
      {...props}
    >
      <nav aria-label="Sidebar navigation">{children}</nav>
    </aside>
  );
};
