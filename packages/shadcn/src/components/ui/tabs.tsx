...
const initTabs = (tabsComponent: HTMLDivElement) => {
  const tablist = tabsComponent.querySelector(
    '[role="tablist"]',
  ) as HTMLElement;
  if (!tablist) return;
  ...
  const getPanelForTab = (tab: HTMLButtonElement) => {
    ...
    return tabsComponent.querySelector(
  ...
  tablist.addEventListener("click", (event) => {
    const clickedTab = (event.target as HTMLElement).closest(
      '[role="tab"]',
    ) as HTMLButtonElement;
...
export const Tabs: FC<TabsProps> = ({
  ...
  const tabsRef = ref || createRef<HTMLDivElement>();
  ...
  const onMount = () => {
    ...
    const instanceId =
      tabsRoot.id || `tabs-${Math.random().toString(36).slice(2, 9)}`;
    if (!tabsRoot.id) {
      tabsRoot.id = instanceId;
    }

    const tablist = tabsRoot.querySelector(
      '[role="tablist"]',
    ) as HTMLElement | null;
    const tabs = tablist
      ? (Array.from(
    ...
    if (defaultValue) {
      const defaultTab = tabsRef.current.querySelector(
        `[role="tab"][data-value="${defaultValue}"]`,
      ) as HTMLButtonElement;
      if (defaultTab) {
        defaultTab.click();
      }
...
export const TabsList: FC<TabsListProps> = ({
  ...
  return (
    <nav role="tablist" class={cn(className)} {...props}>
      {children}
    </nav>
  );
};
