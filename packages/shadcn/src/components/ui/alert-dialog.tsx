...
export type AlertDialogTriggerProps = ElementProps<HTMLButtonElement> & {
  dialogId: string;
};
...
export const AlertDialog: FC<AlertDialogProps> = ({
  className,
  children,
  ref = createRef() as Ref<HTMLDialogElement>,
  ...props
}) => {
  const dialogRef = ref || createRef<HTMLDialogElement>();

  return (
    <dialog ref={dialogRef} class={cn("dialog", className)} {...props}>
      {children}
    </dialog>
  );
};
