...
export const Avatar: FC<AvatarProps> = ({
  className,
  ref = createRef() as Ref<HTMLImageElement>,
  ...props
}) => {
  const avatarRef = ref || createRef<HTMLImageElement>();
...
export const AvatarFallback: FC<AvatarFallbackProps> = ({
  ...
  <span
    class={cn(
      "size-8 shrink-0 rounded-full bg-muted text-muted-foreground inline-flex items-center justify-center text-xs font-medium",
      className,
    )}
    {...props}
  >
...
export const AvatarGroup: FC<AvatarGroupProps> = ({
  ...
  <div
    class={cn(
      "flex -space-x-2 [&_img]:ring-background [&_img]:ring-2 [&_img]:size-8 [&_img]:shrink-0 [&_img]:object-cover [&_img]:rounded-full",
      className,
    )}
    {...props}
  >
