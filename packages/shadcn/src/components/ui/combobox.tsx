...
const initCombobox = (
  ...
  const popover = component.querySelector(
    ":scope > [data-popover]",
  ) as HTMLElement;
  ...
  const hiddenInput = component.querySelector(
  ...
  const filteredOptions = () =>
    options.filter((o) => o.getAttribute("aria-hidden") !== "true");
  const getOptionValue = (option: HTMLElement) =>
  ...
  const initialOption =
    options.find((option) => getOptionValue(option) === hiddenInput.value) ||
    options[0];
  if (initialOption) {
...
export const Combobox: FC<ComboboxProps> = ({
  ...
  const rootRef = ref || createRef<HTMLDivElement>();
