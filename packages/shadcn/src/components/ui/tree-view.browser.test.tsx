...
describe("TreeView", () => {
	...
	describe("Selection (click on row)", () => {
		...
		it("should highlight the selected row", async () => {
			...
			expect(aRow.classList.contains("is-selected")).toBe(true);
		});
	...
	describe("dataview prop", () => {
		it("derives idField from dataview.tree.idField when not explicitly set", async () => {
			const view = makeView(["src"]);
			const entries = applyDataview(sampleData, view);

			const onSelect = vi.fn();

			// Do NOT pass idField explicitly — let the component derive it from dataview
			await $(container).jsx(
				<TreeView
					entries={entries}
					dataview={view}
			...
			const rows = container.querySelectorAll('[role="treeitem"]');
			// Same row count as when idField is explicit
			expect(rows.length).toBeGreaterThan(0);

			// Row data-node-id should use the string id values
			const srcRow = container.querySelector(
				"[data-node-id='\"src\"']",
			) as HTMLElement;
			expect(srcRow).not.toBeNull();

			// Click a child row and verify onSelect called with correct id type
			const aRow = container.querySelector(
				"[data-node-id='\"src/a.ts\"']",
			) as HTMLElement;
			expect(aRow).not.toBeNull();
		...
		it("explicit idField overrides dataview.tree.idField", async () => {
			const numData = [
				{ id: 1, parentId: null, name: "Root" },
			...
			// Pass explicit idField matching the data
			await $(container).jsx(
				<TreeView
					entries={entries}
					dataview={view}
			...
			const childRow = container.querySelector('[data-node-id="2"]') as HTMLElement;
