...
export type CommandMenuHeadingProps = ElementProps<HTMLSpanElement>;
...
const initCommand = (container: HTMLDivElement) => {
  ...
  const filterMenuItems = () => {
    ...
    allMenuItems.forEach((item) => {
      ...
      const itemText = ((item as any).dataset.filter || item.textContent || "")
        .trim()
        .toLowerCase();
  ...
  menu.addEventListener("click", (event) => {
    ...
    if (clickedItem && visibleMenuItems.includes(clickedItem)) {
      const dialog = container.closest(
...
export const Command: FC<CommandProps> = ({
  ...
  const commandRef = ref || createRef<HTMLDivElement>();
...
export const CommandGroup: FC<CommandGroupProps> = ({
  className,
  heading,
  children,
  ...props
...
export const CommandEmpty: FC<CommandEmptyProps> = ({
  className,
  children,
  ...props
}) => {
...
export const CommandSeparator: FC<CommandSeparatorProps> = ({
  className,
  ...props
}) => {
...
export const CommandMenuHeading: FC<CommandMenuHeadingProps> = ({
  className,
  children,
  ...props
}) => {
  return (
    <span role="heading" class={cn(className)} {...props}>
      {children}
    </span>
  );
};
