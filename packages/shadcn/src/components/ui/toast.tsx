...
const initToast = (element: HTMLElement) => {
  ...
  const timeoutDuration =
    duration !== -1
      ? duration ||
        ((element as any).dataset.category === "error" ? 5000 : 3000)
      : -1;
...
const initToaster = (container: HTMLDivElement) => {
  ...
  container.addEventListener("click", (event) => {
    ...
    if (actionLink || actionButton) {
      closeToast(
        (event.target as HTMLElement).closest(".toast") as HTMLElement,
      );
    }
  });
...
export const toast = (config: ToastConfig) => {
  ...
  const footerHtml =
    actionHtml || cancelHtml
      ? `<footer>${actionHtml}${cancelHtml}</footer>`
      : "";
...
export const Toaster: FC<ToasterProps> = ({
  ...
  const toasterRef = ref || createRef<HTMLDivElement>();
...
export const Toast: FC<ToastProps> = ({
  ...
  const toastRef = ref || createRef<HTMLDivElement>();
  ...
  return (
    ...
    >
      <div class="toast-content">
        {category && (
          <span dangerouslySetInnerHTML={{ __html: ICONS[category] }} />
        )}
...
export const ToastTitle: FC<ElementProps<HTMLHeadingElement>> = ({
  className,
  children,
  ...props
}) => (
...
export const ToastDescription: FC<ElementProps<HTMLParagraphElement>> = ({
  className,
  children,
  ...props
}) => (
...
export const ToastAction: FC<ElementProps<HTMLButtonElement>> = ({
  className,
  children,
  ...props
}) => (
