...
describe("DataTable", () => {
	...
	describe("dataview prop", () => {
		it("derives sortField and sortDirection from dataview when not explicitly set", async () => {
			const view = createDataview({
			...
			await $(container).jsx(
				<DataTable entries={entries} columns={columns} dataview={view} />,
			);

			const amtHeader = container.querySelector(
				'th[data-field="amount"]',
			) as HTMLElement;
			expect(amtHeader.textContent).toContain("\u25BC"); // desc indicator
		});

		it("explicit sortField overrides dataview sorters", async () => {
			const view = createDataview({
				sorters: [{ field: "amount", direction: "desc" }],
			});
			const entries = applyDataview(sampleData, view);
			...
			expect(idHeader.textContent).toContain("\u25B2"); // asc indicator on id
			const amtHeader = container.querySelector(
				'th[data-field="amount"]',
			) as HTMLElement;
			expect(amtHeader.textContent).not.toContain("\u25BC"); // no desc on amount
		});

		it("derives idField from dataview for row keying and row click lookup", async () => {
			const customData = [
				{ _id: "x1", name: "Alpha" },
				{ _id: "x2", name: "Beta" },
			];
			...
			const entries = applyDataview(customData, view);
			const customColumns: DataTableColumn[] = [
				{ field: "_id", label: "ID" },
				{ field: "name", label: "Name" },
			];
			...
			// Row data-row-id should be derived from _id field
			expect((rows[0] as HTMLElement).dataset.rowId).toBe("x1");

			// Click row and verify callback
			(rows[0] as HTMLElement).click();
			expect(onRowClick).toHaveBeenCalledWith(
				expect.objectContaining({ row: expect.objectContaining({ _id: "x1" }) }),
			);
		});

		it("re-renders with updated dataview sort indicators on programmatic state change", async () => {
			const tableRef = createRef<HTMLDivElement>();
			...
			// Initially sorted by id asc - indicator on id header
			let idHeader = container.querySelector('th[data-field="id"]') as HTMLElement;
			expect(idHeader.textContent).toContain("\u25B2");

			// Programmatically change sort to amount desc
			store.set({
				view: createDataview({
					sorters: [{ field: "amount", direction: "desc" }],
				}),
			...
			// id header should no longer have an indicator
			idHeader = container.querySelector('th[data-field="id"]') as HTMLElement;
			expect(idHeader.textContent).not.toContain("\u25B2");
			expect(idHeader.textContent).not.toContain("\u25BC");

			// amount header should show desc indicator
			const amtHeader = container.querySelector(
				'th[data-field="amount"]',
			) as HTMLElement;
			expect(amtHeader.textContent).toContain("\u25BC");
