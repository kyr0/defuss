...
type WasiInstance = {
  ...
  wasiImport: Record<string, (...args: number[]) => number>;
...
const dataListeners = new Set<(data: number[]) => void>();
const resizeListeners = new Set<(size: { cols: number; rows: number }) => void>();

(master as any).activate({
  ...
  onData(listener: (data: number[]) => void) {
  ...
  onData(listener: (data: number[]) => void): { dispose(): void };
  onBinary(listener: (data: number[]) => void): { dispose(): void };
...
function memoryView(wasi: WasiInstance) {
  return new DataView(wasi.inst.exports.memory.buffer);
}

function memoryBytes(wasi: WasiInstance) {
  return new Uint8Array(wasi.inst.exports.memory.buffer);
}

function readFromInputRing(maxLen: number): Uint8Array {
...
function forwardInput(payload: number[]) {
  if (payload.length === 0) {
    return;
  }
...
function waitForReadable(timeoutNs?: bigint): boolean {
  pumpInput();
...
function waitForClock(timeoutNs: bigint) {
  if (timeoutNs <= 0n) {
    return;
  }
...
function readFromPty(maxLen: number): Uint8Array {
...
function writeEvent(view: DataView, ptr: number, userdata: bigint, type: number) {
...
const wasi = new WASI(args, env, [], { debug: false }) as WasiInstance;
...
for (const entry of WebAssembly.Module.imports(module)) {
  ...
  if (!(entry.name in wasi.wasiImport)) {
    wasi.wasiImport[entry.name] = (..._args: number[]) => ERRNO_NOSYS;
  }
}
...
function noteInputChunk(data: Uint8Array) {
...
wasi.wasiImport.fd_filestat_get = (fd: number, filestatPtr: number) => {
  if (fd > 2) {
...
wasi.wasiImport.poll_oneoff = (inPtr: number, outPtr: number, nsubscriptions: number, neventsPtr: number) => {
  ...
  let stdinSubscription: wasiTypes.Subscription | undefined;
  let clockSubscription: wasiTypes.Subscription | undefined;
...
wasi.wasiImport.proc_exit = (code: number) => {
...
if (DEBUG) {
  ...
  for (const name of importedFunctionNames) {
    ...
    wasi.wasiImport[name] = (...syscallArgs: number[]) => {
